...
def load_topics() -> List[Dict]:
    ...
    try:
        ...
        with open(course_list_path, 'r', encoding='utf-8') as f:
            return json.load(f)['python']['topics']
...
class AssignmentDataManager:
    ...
    def get_answer(self, topic_id: str, assignment_id: int) -> Optional[Dict]:
        ...
        return next((a for a in answers['assignments'] if a['id'] == assignment_id), None)
...
class CodeSubmissionAnalyzer:
    ...
    async def analyze_submission(self, submitted_code: str,
    ...
    async def _analyze_with_llm(self, submitted_code: str,
                               expected_solution: str,
                               test_results: Dict,
                               topic_id: str) -> Dict:
        """LLM을 사용한 코드 분석"""
...
"""
    ...
    async def analyze_code_analysis(self, submitted_analysis: str,
                                  code_to_analyze: str,
                                  points_to_consider: List[str],
                                  expected_points: List[str]) -> CodeAnalysisResult:
        """코드 분석 답안 평가"""
        try:
            # LLM을 사용한 분석 평가
...
class AssignmentGenerator:
    ...
    async def generate_assignment(self, topic_id: str,
                                topic_name: str,
                                assignment_type: str,
                                difficulty: str = 'beginner') -> Dict:
        """과제 생성"""
        system_prompt = self._get_system_prompt(topic_id, topic_name, difficulty)
        prompt = PROMPTS[assignment_type].format(topic_name=topic_name)
        response = await self.llm.agenerate([system_prompt, prompt])
        return json.loads(response.content)

    def _get_system_prompt(self, topic_id: str, topic_name: str, difficulty: str) -> str:
        """시스템 프롬프트 생성"""
        if topic_id == "input_output":
            return """당신은 Python 교육 전문가입니다.
...
5. file, open, read, write 등의 내용은 절대 포함하지 않습니다
...
4. 테스트 케이스는 기본/경계/예외 상황 포함"""
    ...
    async def generate_topic_assignments(self, topic_id: str,
                                       topic_name: str) -> Dict:
        """토픽의 전체 과제 세트 생성"""
        assignments = []
